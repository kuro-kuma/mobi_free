...
export const useBluetooth = () => {
	const [A,O] = useState(!1)
	      ...
	      , R = useCallback(async () => {
	        try {
	            d(null);
	            const N = navigator.bluetooth;
	            if (!window.isSecureContext || !N?.requestDevice)
	                throw new Error("Web Bluetooth API 不可用。请使用支持的浏览器（Chrome、Edge、Opera）访问，iOS 用户请使用 Bluefy 浏览器。");
	            let W;
	            try {
	                W = await N.requestDevice({
	                    filters: [{
	                        services: [Zn]
	                    }],
	                    optionalServices: [Zn]
	                ...
	                if (!(gl instanceof Error && gl.name === "NotFoundError"))
	                    throw gl;
	                W = await N.requestDevice({
	                    acceptAllDevices: !0,
	                    optionalServices: [Zn]
	            ...
	            const F = await (await W.gatt?.connect())?.getPrimaryService(Zn);
	            if (!F)
	                throw new Error("未发现 FTMS 服务");
	            const rl = await F?.getCharacteristic(Jv);
	            await rl?.startNotifications(),
	            rl?.addEventListener("characteristicvaluechanged", gl => {
	                const _l = gl.target.value;
	                if (!_l)
	                    return;
	                const Ll = Kv(_l);
	                C(ot => ({
	            ...
	            const Rl = await F?.getCharacteristic(wv);
	            await Rl?.startNotifications(),
	            Rl?.addEventListener("characteristicvaluechanged", gl => {
	                gl.target.value
	            }
	            ...
	            L.current = Rl || null,
	            await Rl?.writeValue(new Uint8Array([0])),
	            await new Promise(gl => setTimeout(gl, 100)),
	            await Rl?.writeValue(new Uint8Array([7])),
	            ml.current = W,
	            O(!0),
	            W.addEventListener("gattserverdisconnected", () => {
	                O(!1),
	                L.current = null
	            }
	            )
	            ...
	            console.error("蓝牙连接失败:", N)
	        }
	    }
	    , [])
	      , T = useCallback( () => {
	        ml.current?.gatt?.disconnect()
	    }
	    , [])
	      , w = useCallback(async N => {
	        if (!L.current)
	            return;
	        const Al = Math.max(10, Math.min(24, Math.round(N)))
	          , F = new Uint8Array([4, Al & 255, Al >> 8 & 255]);
	        try {
	            await L.current.writeValue(F),
	            "vibrate"in navigator && navigator.vibrate(50)
	        } catch (rl) {
	            console.error("阻力设置失败:", rl)
	        }
	    }
	    , []);
	    return {
